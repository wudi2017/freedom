...
public class ClearWorkRequest extends BQThreadRequest {
	...
	public void doAction() {
		...
		for(int i=0; i<stockIDHoldList.size(); i++)
		{
					...
					&& 0 == cResultStockTime.error)
			{
				...
				else
				{
					...
					BLog.output("CLEAR", "    GetHoldStock %s %s %s failed, ignore\n", m_date, m_time, stockID);
				}
			}
			else
			{
		...
		for(int i = 0; i< cClearResultWrapperList.size(); i++)
		{
			...
			// 调用账户获取持有量
			Vector<HoldStock> holdStockVector = new Vector<HoldStock>();
			int iRetHoldStock = accIF.getHoldStock(null, null, cClearResultWrapper.stockId, holdStockVector);
			if(0 == iRetHoldStock)
			{
				HoldStock cHoldStock = null;
				if(holdStockVector.size()==1)
				...
				{
					BLog.output("CLEAR", "    cHoldStock null\n");
				}
			}
			else
			{
				BLog.output("CLEAR", "    getHoldStock failed\n");
			}
		}
